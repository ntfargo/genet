...
ifeq ($(USE_CPU),true)
	$(CONDA_ACTIVATE) $(BINARY_NAME) && pip install torch==2.0.0+cpu -f https://download.pytorch.org/whl/cpu
else ifeq ($(USE_CUDA),true)
	$(CONDA_ACTIVATE) $(BINARY_NAME) && pip install torch==2.0.0+cu118 -f https://download.pytorch.org/whl/apple/cpu/torch_stable.html
